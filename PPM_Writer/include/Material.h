...
{
public:
	Material() : m_albedo(1.f, 1.f, 1.f), m_ambient(0.f), m_diffuse(0.f), m_specular(0.f), m_roughness(0.f), m_reflective(0.f) {};
	Material(const Vector3& a_albedo, float a_ambient, float a_diffuse, float a_specular, float a_rooughness, float a_reflective = 0.f):
		m_albedo(a_albedo), m_ambient(a_ambient), m_diffuse(a_diffuse), m_specular(a_specular), m_roughness(a_rooughness), m_reflective(a_reflective) {};
	~Material() = default;
